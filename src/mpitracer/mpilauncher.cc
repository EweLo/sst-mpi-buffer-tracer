...
int main(int argc, char *argv[]) {
    ...
    // Check if rankfile.txt exists
    if (access("rankfile.txt", F_OK) == 0) {
        // Add rankfile to command
        mpi_cmd << " --rankfile rankfile.txt"; // TODO deprecated
    }
    ...
    const char* trace_lib_path = TRACING_LIB_PATH;
    if (access(trace_lib_path, F_OK) == -1) {
        printf("Error: mpilauncher.cc: Tracing library %s not found.\n", trace_lib_path);
        exit(-1);
    }

    // Set LD_PRELOAD
    setenv("LD_PRELOAD", trace_lib_path, 1);
